<template>
  <div class="py-2 mt-12">
    <div class="rounded bg-gray-900 text-gray-300 p-6 flex justify-start">
      <img :src="album.image" class="mb-3 rounded-full h-24 w-24 object-cover">
      <div class="div ml-4 flex flex-col justify-center">
        <h1 class="text-2xl font-semibold">{{album.title}}</h1>
        <h4 class="text-lg font-semibold">Album</h4>
        <button @click="goBack" class="rounded bg-blue-500 px-2 py-1 mt-3">All Album</button>
      </div>
...
export default {
  ...
  methods: {
    getAlbum(){
      axios.get(`http://localhost:8000/api/album/${this.albumid}`, {headers:{
        'Content-Type': 'application/json',
        'Accept': 'application/json', 
        'Authorization': 'Bearer '+this.newtoken,}})
        .then(res => {
          this.album = res.data
          this.getSongs(this.album.title)
        })
        .catch(err => console.log(err))
    },

    getSongs(name){
      axios.get(`http://localhost:8000/api/song/search/album/${name}`, {headers:{
        'Content-Type': 'application/json',
        'Accept': 'application/json', 
        'Authorization': 'Bearer '+this.newtoken,}})
        .then(res => {
